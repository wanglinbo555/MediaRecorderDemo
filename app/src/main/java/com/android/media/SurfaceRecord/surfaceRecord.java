...
/**
 * Created by xa on 2018/4/7.
 */

public class surfaceRecord {
    private static final String TAG = "surfaceRecord";
    ...
    public surfaceRecord(Camera camera){
        Log.i(TAG,"surfaceRecord init .");
        mCamera = camera;
    }
    ...
    public void updatePresentationTime(long startWhen){
        long timestamp = mStManager.getSurfaceTexture().getTimestamp();
        if (VERBOSE) {
            Log.d(TAG, "present: " +
                    ((timestamp - startWhen) / 1000000.0) + "ms");
        }
     ...
     */
    public void prepareEncoder(int width, int height, int bitRate, String path) {
        ...
        if(path == null || "".equals(path)){
            outputPath = new File(OUTPUT_DIR,
